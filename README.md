...
**[JobTrack Extension]( cazazolatatiana/JobTracker_App)** — the Chrome sidebar that captures job postings and logs decisions. Built with Plasmo. Covers the page-scraping strategy per job board, the Edge Functions it calls, and known limitations.

**[JobTrack Dashboard](cazazolatatiana/resume-road-map)** — the pipeline management interface. Built with TanStack Start, React, and Supabase. Covers the full database schema, the two-stage AI pipeline in detail, and setup instructions.
